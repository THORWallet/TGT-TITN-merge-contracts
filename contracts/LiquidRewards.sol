...
interface ILiquidRewardsERC20Metadata is IERC20 {
    function name() external view returns (string memory);
    function symbol() external view returns (string memory);
    function decimals() external view returns (uint8);
}

contract LiquidRewards {
    ILiquidRewardsERC20Metadata private _titnContract;
    ...
    constructor(address titnContract, uint64 startTimestamp, uint64 vestingDuration) {
        _owner = msg.sender;
        _titnContract = ILiquidRewardsERC20Metadata(titnContract);
    ...
    function vestingDetails(address account) public view virtual returns (uint96 totalAmount, uint96 claimed, uint256 claimable) {
        VestingParams memory v = _vesting[account];
        totalAmount = v.vestingAmount;
        claimed = v.vestingClaimed;
        if (block.timestamp > _startTimestamp && _startTimestamp != 0) {
            claimable = claimableAmount(v);
        }
    }
    ...
    function cancel(address account) public virtual onlyOwner {
        require(account != address(0), "Vesting: cancel to the zero address");

        VestingParams storage v = _vesting[account];
        require(v.vestingAmount > 0, "Vesting: no active vesting for this account");

        uint256 claimable = claimableAmount(v);
        uint256 unvested = v.vestingAmount - claimable;

        delete _vesting[account];
        _vestedBalance -= (claimable + unvested);

        if (claimable > 0) {
            _titnContract.transfer(account, claimable);
            emit Claim(account, uint96(claimable));
        }

        if (unvested > 0) {
            _titnContract.transfer(_owner, unvested);
        }
    }

    function recoverTokens() public virtual onlyOwner {
        require(block.timestamp >= _startTimestamp + 365 days, "LiquidRewards: can only recover after 1 year");

        uint256 balance = _titnContract.balanceOf(address(this));
        require(balance > 0, "LiquidRewards: no tokens to recover");

        _vestedBalance = 0;
        _titnContract.transfer(_owner, balance);
    }
}
